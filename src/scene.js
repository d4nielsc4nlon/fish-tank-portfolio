...
/**
 * Prevent double-init if multiple modules import scene.js on a page.
 * (Also helps if hot-reload imports twice in dev.)
 */
const GLOBAL_KEY = "__DANIEL_TANK_SCENE__";
...
float causticField(vec2 uv, float t){
  ...
  // Drift mostly along +V direction (screen "toward camera" feel)
  uv += vec2(0.0, t * 0.22);
...
function initScene() {
  // Avoid double init
  if (window[GLOBAL_KEY]?.initialized) return;
  window[GLOBAL_KEY] = { initialized: true };
  ...
  const camera = new THREE.PerspectiveCamera(60, vp0.w / vp0.h, 0.1, 200);
  camera.position.set(0, -1, 12);
  camera.lookAt(0, 0, 0);

  // We'll use pointer position for subtle parallax
  const mouse = { x: 0, y: 0 };
 ...
 Object.assign(renderer.domElement.style, {
  ...
  display: "block",
  zIndex: "0",
  pointerEvents: "none",   // ✅ critical: don’t block scroll/click
});

  // ------------------------------
  // Lighting (same as main.js)
  ...
  const causticsShadowMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main(){
        ...
        // fade caustics out toward far edge
  ...
  const causticsHighlightPlane = new THREE.Mesh(
    floorGeo.clone(),
    causticsHighlightMat
  );
  ...
  // ------------------------------
  // Postprocessing shaders (same as main.js)
  ...
  // ------------------------------
  // Pointer events: parallax + ripple tap
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      ripplePass.uniforms.center.value.set((mx + 1) / 2, (my + 1) / 2);
  ...
  function animate() {
    ...
    void delta; // reserved if you add mixers later
    const t = performance.now() * 0.001;
    const baseTime = t * 0.25;

    // camera parallax (same logic as main, but no fish raycasting)
  ...
  // Expose a tiny handle if you ever want it
  window[GLOBAL_KEY].api = { renderer, scene, camera, composer };
}

// Run reliably even when Vite injects scripts into <head> in production
