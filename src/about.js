...
// ------------------------------
// Sea floor dune animation cache
...
const OverheadRipplesShader = {
  ...
  fragmentShader: `
    ...
    void main(){
      ...
      p.x *= aspect;
      p *= scale;
      p += vec2(0.0, time * speed * 2.0);

      float c1 = causticField(p * 0.85, time * 0.75);
      float c2 = causticField(p * 0.62, time * 0.75 + 9.0);
...
// Optional click ripple, but no navigation/fish interaction
window.addEventListener('pointerdown', (e) => {
  ...
  ripplePass.uniforms.center.value.set((x + 1) / 2, (y + 1) / 2);
  ripplePass.uniforms.strength.value = 0.02;
  ...
  function fadeRipple() {
    ...
    ripplePass.uniforms.strength.value = 0.02 * (1 - progress);

    if (progress < 1) requestAnimationFrame(fadeRipple);
  }
